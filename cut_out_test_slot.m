...
for i = 1 : how_many_tasks
    ...
    test_slot_cell{i}.TimeStamp_lidar = task_temp.TimeStamp_lidar(test_start_index_lidar:test_end_index_lidar);
    ...
    for j=1:frames_number_in_current_slot
        ...
        if isempty(x_y_coordinatetemp) || size(x_y_coordinatetemp,1)==1
            continue;
        end        
        D = pdist2(x_y_coordinatetemp,x_y_coordinatetemp);%Pairwise distances between observations
        [idx, corepts] = dbscan(D,1,10,'Distance','precomputed'); % corepts means core points
        ...
        % other wierd clusters, but in reality you never know.
        if max(idx) == 1
            ...
            % as our participant
            expect_convexhull_size = 0.075; %!!!!!!!!!!will be modified later
    ...
    % there will still be several NaN data in
    % current_slot_trajectory_reconstruction after the operation we have
    % done above, so we fillmissiing them
...
%     ==========================
%     % in frames_number_in_current_slot x 2 format
...
%     % is 0.05s
%     x_y_velocity_from_lidar = [[0,0];diff(current_slot_trajectory_reconstruction,1,1)]/0.05;
%     test_slot_cell{i}.x_y_velocity_from_lidar = x_y_velocity_from_lidar;
%     ==========================
%%
% the above diff method doesn't show a good result in derivating the
% speed, so we change to the customdefined function below, it has a very
% slight improvement but not significant. There is not really difference
% between diff. But still, we keep our compute_speed_from_trajectory_reconstruction
